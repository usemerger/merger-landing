...
export default function ProfilePage() {
  ...
  return <Shell authed><main className="dash-main">
    ...
    {access && <>
      <form className="panel mt-24" onSubmit={save} aria-busy={busy === 'profile'}><h2>Your details</h2><div className="field mt-24"><label htmlFor="display-name">Name</label><input type="text" id="display-name" autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} maxLength={100} required disabled={Boolean(busy)} /></div><div className="field"><label htmlFor="account-email">Email</label><input type="text" id="account-email" value={access.user.email} readOnly /><p className="field-hint">{access.user.emailVerified ? 'Verified' : 'Verification required'}. Contact support if you need to change your email.</p></div><button className="btn btn-primary btn-sm" type="submit" disabled={Boolean(busy)}>{busy === 'profile' ? 'Saving…' : 'Save profile'}</button></form>
