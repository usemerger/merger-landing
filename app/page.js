  ...
  return <div className="marketing-page">
    ...
    <main id="main-content">
      <section className="mk-hero" aria-labelledby="hero-title">
        ...
        <div className="mk-wrap mk-hero-grid">
          <div className="mk-hero-copy">
            ...
            <p className="mk-hero-price"><strong>Free during the alpha</strong> <span>·</span> Card required, {ALPHA_OFFER.todayLabel} charged today · Invite-only</p>
          </div>
      ...
      <section id="pricing" className="mk-pricing mk-section" aria-labelledby="pricing-title"><div className="mk-wrap">
        <div className="mk-section-heading"><div><p className="mk-kicker">An early seat at the desk</p><h2 id="pricing-title">Free during the alpha.<br /><span>Then half price, for good.</span></h2></div><p>Join the Windows alpha at no cost. When it ends, members keep {ALPHA_OFFER.memberPrice} a month against a {ALPHA_OFFER.listPrice} list price. Your Anthropic API usage and DocuSign account are separate.</p></div>
        <div className="mk-pricing-grid"><article className="mk-alpha-plan"><div className="mk-plan-heading"><div><span className="mk-kicker">Windows desktop</span><h3>{ALPHA_OFFER.name}</h3></div><span className="mk-plan-badge">Invite-only</span></div><div className="mk-plan-price"><strong>{ALPHA_OFFER.todayLabel}</strong><span>{ALPHA_OFFER.todayNote}<small>then {ALPHA_OFFER.memberPrice} {ALPHA_OFFER.intervalLabel} · half of {ALPHA_OFFER.listPrice}</small></span></div><p className="mk-plan-rate">{ALPHA_OFFER.rateNotice}</p><ul className="mk-plan-features"><li>Connected messages in one workspace</li><li>Deal Desk and Rolodex</li><li>Claude features with your Anthropic API key</li><li>DocuSign in the app with your own account</li></ul><Link className="mk-button" href={ALPHA_OFFER.signupHref}>{ALPHA_OFFER.checkoutLabel}</Link><p className="mk-plan-billing">{ALPHA_OFFER.billingNotice}</p><p className="mk-plan-extras">Anthropic API usage and DocuSign charges are not included.</p></article>
