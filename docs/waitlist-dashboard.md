...
The admin API returns at most 20,000 combined queued and pending entries, with `totalCount` and `truncated`. If capped, the UI labels the partial list and its scoped search/counts. This change does not add global pagination.
