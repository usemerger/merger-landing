...
scenes=(root/'scenes.mjs').read_text(encoding='utf-8').replace("import brandIcons from './channel-icons.mjs';",'').replace("import {createMessagingScenes} from './messaging-scenes.mjs';",'').replace('export const ','const ').replace('export function ','function ')
messaging=(root/'messaging-scenes.mjs').read_text(encoding='utf-8').replace('export function ','function ')
